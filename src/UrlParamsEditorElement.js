 ...
 */
export class UrlParamsEditorElement extends ArcResizableMixin(ArcOverlayMixin(ValidatableMixin(LitElement))) {
  static get styles() {
    return styles;
  }
   ...
   */
  _getValidity() {
    const inputs = /** @type AnypointInput[] */ (Array.from(this.shadowRoot.querySelectorAll('.params-list anypoint-input')));
    let result = true;
  ...
  [paramNameInput](item, index) {
    ...
    <anypoint-input
      ...
      ?readOnly="${readOnly}"
      pattern="\\S*"
      @value-changed="${this[paramInputHandler]}"
      noLabelFloat
    >
